 ...
 * @category Common
 */
class Chown extends CommonModule
{
	/**
	 * Base command to run.
	 */
	private const CMD = 'chown';
	 ...
	 * Execute command.
	 *
	 * @param string $file file path to set ownership
	 * @param string $chuser user to set on file/dir
	 * @param string $chgroup group to set on file/dir
	 * @param array $params command parameters
	 * @param int $ptype command pattern type
	 * @return array command output
	 */
	public function execCommand(string $file, string $chuser = '', string $chgroup = '', array $params = [], int $ptype = self::PTYPE_REG_CMD): array
	{
		...
		if ($result['exitcode'] != 0) {
			...
			$emsg = "Error while setting ownership to '{$file}': ExitCode: {$result['exitcode']}, Output: '{$out}'";
			Logging::log(
	...
	private function prepareCommand(string $file, string $chuser = '', string $chgroup = '', array $params = [], int $ptype = self::PTYPE_REG_CMD)
	{
		...
		$chown_cmd = $this->getCmdPattern($ptype);
		$cmd = sprintf(
			$chown_cmd,
			self::CMD,
			$options,
