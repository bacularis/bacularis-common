 ...
 * @category Module
 */
class DataDependencies extends CommonModule
{
	/**
	 * Data dependencies file
	 */
	public const DATA_DEPS_FILE = 'Bacularis.Common.Data.data_deps';
	...
	{
		$dependent = false;
		if ($resource_type === 'Schedule' && key_exists('Run', $resource)) {
			for ($i = 0; $i < count($resource['Run']); $i++) {
				if (key_exists($deps_directive, $resource['Run'][$i]) && $resource['Run'][$i][$deps_directive] === $resource_name) {
					$dependent = true;
					break;
				}
			}
		} else {
			if (key_exists($deps_directive, $resource)) {
				...
				for ($i = 0; $i < count($config); $i++) {
					foreach ($config[$i] as $rctype => $resource) {
						...
						for ($j = 0; $j < count($directives); $j++) {
							...
							$result[] = [
								'component_type' => $misc->getComponentFullName($component_type),
