...
/**
 * The MySQL backup plugin module.
 *
 * @author Marcin Haba <marcin.haba@bacula.pl>
 * @category Plugin
 */
class MySQLBackup extends BacularisCommonPluginBase implements IBaculaBackupFileDaemonPlugin
{
	/**
	 * SQL dump backup tools.
	 */
	private const SQL_DUMP_PROGRAM = 'mysqldump';
	private const SQL_CLI_PROGRAM = 'mysql';
	private const SQL_BINLOG_PROGRAM = 'mysqlbinlog';

	/**
	 * Binary physical backup tools.
	 */
	private const BIN_BACKUP_PROGRAM = 'xtrabackup';
	private const BIN_STREAM_PROGRAM = 'xbstream';
	...
	/**
	 * Common parameters for the binary physical backup program.
	 */
	private const BINARY_PROGRAM_COMMON_OPTS = [
		'apply-log-only' => true
	];
	 ...
	 * System data params.
	 */
	private const SYSTEM_USERS = 'users';
	...
	public static function getParameters(): array
	{
		return [
			[
				...
				'type' => 'string',
				'default' => 'main',
				'label' => 'User defined MySQL server name (any string)',
	...
	private function getDumpBackupLevelCommand(array $args, string $level): array
	{
		...
		if ($level == 'Full') {
			if (key_exists(self::ALL_DATABASES_ONE_DUMP, $args) || key_exists(self::ALL_DATABASES_LIST, $args)) {
				$cmds[] = $this->getSinglePluginCommand($args, self::ACTION_SYSTEM, self::SYSTEM_USERS);
			}
		}
	...
	private function doFullDumpBackup(array $args): bool
	{
		...
		if (key_exists('databases', $args)) {
			...
			$user_grants = $this->prepareUserGrants($args);
			echo implode(PHP_EOL, $user_grants);
			return true;
		}
	...
	private function getDatabases(array $args): array
	{
		...
		$cmd[] = '"SHOW DATABASES"';
		/**
		 * This is required to not break the command to list plugin commands.
		 * If a --password option used, there is returned a warning from mysql binary.
		 */
		$cmd[] = ' 2>/dev/null';
		$bin = $this->getBinPath($args, self::SQL_CLI_PROGRAM);
		array_unshift($cmd, $bin);
	 ...
	 * @return array get database command output and exit code
	 */
	private function prepareUserGrants(array $args): array
	{
		// Prepare general parameters
		$cmd_params = $this->getGeneralParams($args);
		...
		$cmd = $this->prepareCommandParameters($cmd_params);
		$cmd[] = '--execute';
		$cmd[] = "\"SELECT CONCAT('SHOW CREATE USER ''', user, '''@''', host, '''; SHOW GRANTS FOR ''', user, '''@''', host, ''';') AS query FROM mysql.user\"";
		$bin = $this->getBinPath($args, self::SQL_CLI_PROGRAM);
		array_unshift($cmd, $bin);
		...
		$result = $this->execCommand($cmd);
		if ($result['exitcode'] == 0) {
			$show_grants_cmds = $result['output'];
			// Save show commands in a file to execute
			[$tmpdir, $tmpprefix] = $this->getFileParams('show-grants');
			$tmpfname = tempnam($tmpdir, $tmpprefix);
			$show_grants_list = implode(PHP_EOL, $show_grants_cmds);
			if (file_put_contents($tmpfname, $show_grants_list) === false) {
				Plugins::log(Plugins::LOG_ERROR, "Unable to write to '{$tmpfname}' file.");
				return false;
			}
			$cmd = $this->prepareCommandParameters($cmd_params);
			$bin = $this->getBinPath($args, self::SQL_CLI_PROGRAM);
			array_unshift($cmd, "cat '$tmpfname'", '|', $bin);

			$result = $this->execCommand($cmd);
			if ($result['exitcode'] == 0) {
				$user_grants = $result['output'];
			} else {
				Plugins::log(Plugins::LOG_ERROR, "Error while preparing user grants from file: '{$tmpfname}.");
			}
			unlink($tmpfname);
		}
		return $user_grants;
	...
	private function prepareBinaryBackup(array $args, string $backup_dir): bool
	{
		...
		$cmd_params = array_merge($cmd_params, self::BINARY_PROGRAM_COMMON_OPTS);
		$cmd = $this->prepareCommandParameters($cmd_params);
		$bin = $this->getBinPath($args, self::BIN_BACKUP_PROGRAM);
		array_unshift($cmd, $bin);
	...
	private function getGeneralParams(array $args): array
	{
		...
		if (key_exists('login-path', $args)) {
			$params['login-path'] = $args['login-path'];
		}
